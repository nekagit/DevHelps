...
const executeGitScript = (scriptName, branchName) => {
  return new Promise((resolve, reject) => {
    ...
    exec(`"${scriptPath}" "${branchName}"`, (error) => {
      if (error) {
        reject({
          success: false,
...
app.post("/execute-git-script", async (req, res) => {
  try {
    const { scriptName, branchName } = req.body;

    const result = await executeGitScript(scriptName, branchName);
    if (result.success) {
