...
const executeScriptsSequentially = async (scriptName, scriptParameter, pathDev,pathProj) => {
  const gitCommand = `start cmd /b /c "cd /d ${pathDev}\\src\\scripts && ${scriptName} ${pathProj} ${scriptParameter}"`;
  const npmCommand = `start cmd /b /c "cd /d ${pathDev}\\src\\scripts && ${scriptName} ${pathProj} ${scriptParameter} && pause"`;
