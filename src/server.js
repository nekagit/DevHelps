...
const executeScriptsSequentially = async (scriptName, scriptParameter, pathDev,pathProj) => {
  const gitCommand = `start /min cmd /c "cd /d ${pathDev}\\src\\scripts && ${scriptName} ${pathProj} ${scriptParameter}"`;
  const npmCommand = `start /min cmd /c "cd /d ${pathDev}\\src\\scripts && ${scriptName} ${pathProj} ${scriptParameter} && pause"`;
